...
def topk_sampling(model, prompt, tokenizer, device, max_length = 30, top_k = 50, temperature = 0.8, repetition_penalty = 1.2):
    ...
    # Switch model to inference mode so dropout/etc. stay deterministic during decoding.
    model.eval()

    # Tokenize the prompt into fixed-length tensor (batch=1, seq=max_length) on target device.
    ...
    # Start decoder with BOS token; grows autoregressively as we sample more tokens.
    target_ids = torch.tensor([[tokenizer.bos_token_id]], device= device)

    # Build source mask where padded tokens are zeroed out (so encoder ignores them).
    ...
    with torch.no_grad():
        # Encode the prompt once; decoder will attend to this frozen memory.
        encoder_output = model.encoder(input_ids, src_mask)
    
    for i in range(max_length):
        with torch.no_grad():

            # Run a decoder step using the tokens sampled so far (target_ids).
            decoder_output = model.decoder(encoder_output, encoder_output, target_ids, src_mask, None)

            # Project decoder states to vocabulary logits and grab only latest position.
            logits = model.linear_layer(model.norm(decoder_output))

            logits = logits[:, -1, :] / temperature

            # Convert logits -> probabilities across vocabulary.
            probs = F.softmax(logits, dim = -1)

            # Keep only the top_k most likely tokens to ensure sampling stays focused.
            top_k_probs, top_k_indices = torch.topk(probs, top_k, dim=-1)

            # Sample one token from the truncated distribution (categorical sampling).
            next_token = torch.multinomial(top_k_probs, num_samples=1)

            # Map sampled index back to the actual vocabulary id.
            xcol = torch.gather(top_k_indices, -1, next_token)

            # Append the new token to the running target sequence for the next iteration.
            target_ids = torch.cat([target_ids, xcol], dim=1)

            # Stop early once EOS is produced.
        ...
        # Decode token ids back to human-readable text after generation finishes.
